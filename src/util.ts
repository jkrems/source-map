...
export function getArg<T>(aArgs: unknown, aName: string): T;
export function getArg<T>(aArgs: unknown, aName: string, aDefaultValue: T): T;
 ...
 */
export function getArg<T>(aArgs: any, aName: string, aDefaultValue?: T) {
  if (aName in aArgs) {
    return aArgs[aName];
 ...
 */
function toSetStringFallback(aStr: string): string {
...
export const toSetString = supportsNullProto ? identity : toSetStringFallback;

function fromSetStringFallback(aStr: string): string {
  if (isProtoString(aStr)) {
    return aStr.slice(1);
  }
...
export const fromSetString = supportsNullProto ? identity : fromSetStringFallback;

function isProtoString(s: string): boolean {
  if (!s) {
    return false;
  }
...
function strcmp(aStr1: string, aStr2: string): number {
  if (aStr1 === aStr2) {
    return 0;
  }
 ...
 */
export function compareByGeneratedPositionsInflated(mappingA: Mapping, mappingB: Mapping): number {
 ...
 * JSON.
 */
export function parseSourceMapInput(str: string) {
 ...
 */
function createSafeHandler(cb: (url: URL) => unknown) {
  return (input: string) => {
...
function buildUniqueSegment(prefix: string, str: string): string {
  let id = 0;
  do {
...
function buildSafeBase(str: string): string {
...
type URLType = "absolute" | "path-absolute" | "path-relative" | "scheme-relative";
 ...
 */
function computeRelativeURL(rootURL: string|URL, targetURL: string|URL): string {
 ...
 */
const ensureDirectory = createSafeHandler((url: URL) => {
 ...
 * @return A normalized URL value.
 */
const trimFilename = createSafeHandler((url: URL) => {
 ...
 * @return A normalized URL value.
 */
export const normalize = createSafeHandler((url: URL) => {});
 ...
 */
export function join(aRoot: string, aPath: string): string {
 ...
 */
export function relative(rootURL: string, targetURL: string) {
  const result = relativeIfPossible(rootURL, targetURL);

  return typeof result === "string" ? result : normalize(targetURL);
}

function relativeIfPossible(rootURL: string, targetURL: string) {
